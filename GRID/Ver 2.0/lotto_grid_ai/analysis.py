...
class LottoAnalyzer:
    ...
    RECENT_WINDOWS: Tuple[Tuple[int, float], ...] = (
        ...
        (50,    0.18),  # 중기 패턴
        (100,   0.14),  # 중장기 패턴
        (200,   0.08),  # 장기 패턴 (start_draw에 따라 달라짐)
        (500,   0.05),  # 초장기 패턴 (start_draw에 따라 달라짐)
        (10000, 0.02),  # 선택된 전체 회차 범위 (start_draw 기준)
    )
    # ↑ 총합 = 0.15+0.18+0.20+0.18+0.14+0.08+0.05+0.02 = 1.00
    # 200·500·전체 윈도우 추가 → start_draw 변경 시 점수가 실제로 달라짐
