...
class CombinationGenerator:
    ...
    def _search(
        ...
        for seq, combo in enumerate(itertools.combinations(sorted(pool), 6)):
            metrics = self._combo_metrics(combo)
            if not self._passes_math_filters(metrics, config):
                continue
    ...
    @staticmethod
    def _passes_math_filters(metrics: Dict[str, int], config: AnalysisConfig) -> bool:
        return (
            config.sum_min <= metrics["total"] <= config.sum_max
            and config.odd_min <= metrics["odd_count"] <= config.odd_max
            and config.high_min <= metrics["high_count"] <= config.high_max
